...
function renderTabs() {
  $('#tabNav').innerHTML = [
    ...
    ...CLASS_KEYS.map(k => ({ key: k, label: CLASS_META[k].label, count: classItems(k).length, hidden: isClassHidden(k) })),
  ].map(t => `<button class="tab-btn ${t.key === state.activeTab ? 'active' : ''} ${t.hidden ? 'tab-hidden' : ''}" data-tab="${t.key}">
    ${t.label}${t.count !== null ? `<span class="tab-count">${t.count}</span>` : ''}</button>`).join('');
}

function renderPanels() {
  let html = wrap('overview', renderOverview());
  for (const key of CLASS_KEYS) html += wrap(key, renderClassPanel(key));
...
function renderOverview() {
  ...
  const chartData = populated.map(k => {
    ...
    return { key: k, label: CLASS_META[k].label, color: CLASS_META[k].color, icon: CLASS_META[k].icon,
      value: hidden ? 0 : (total ?? count * 0.01), count, hasPrices: total !== null, total, hidden };
  });

  let html = '';

  const warning = allocationWarning();
  if (warning) html += notice('triangle-alert', `As metas somam <strong>${warning.sum}%</strong>, mas deveriam totalizar 100%.`, 'warning');

  if (hasCachedPrices() && pricesStale())
    html += notice('clock', `Cotações desatualizadas (${pricesDateStr()}). Atualize clicando em <strong>Cotar</strong>.`, 'info');
  else if (!hasCachedPrices())
    html += notice('clock', 'Nenhuma cotação carregada. Clique em <strong>Cotar</strong> para buscar preços.', 'info');

  html += '<div class="overview-grid"><div class="chart-card">';
  ...
  html += '</div></div><div class="summary-cards">';

  populated.filter(k => !isClassHidden(k)).forEach(key => {
    const m = CLASS_META[key], total = classTotalBRL(key);
    ...
    html += `<div class="summary-card" style="--card-color:${m.color}">
      ...
      <div class="summary-card-meta">
        ...
        <span class="summary-card-target-wrap">meta <input class="summary-card-target" type="text" value="${target.toFixed(0)}"
          data-class-target="${key}" inputmode="decimal" title="Meta da classe (%)">%</span>
      </div>
    </div>`;
  });
...
function renderDonut(segments) {
  ...
  const arcs = segments.map(d => {
    ...
    const arc = `<circle cx="100" cy="100" r="${R}" fill="none" stroke="${d.color}"
      ...
      <title>${d.label}: ${formatBRL(d.value)} (${pct}%)</title></circle>`;
    offset += dash;
    return arc;
  });
...
function renderClassPanel(key) {
  const meta = CLASS_META[key], items = classItems(key);
  ...
  let html = `<div class="asset-section-header">
    <h2 class="asset-section-title" style="color:${meta.color}">
      <i data-lucide="${meta.icon}" class="section-icon"></i> ${meta.label}</h2></div>`;
  ...
  html += `<div class="table-wrap"><table class="asset-table"><thead><tr>
    ...
    <th class="col-r">Meta %</th><th class="col-action"></th></tr></thead><tbody>`;
  ...
  items.forEach((item, idx) => {
    ...
    if (key === 'fixedIncome' || key === 'assets' || (key === 'storeOfValue' && !p)) {
      ...
      priceStr = (p.currency === 'USD' ? '$ ' : 'R$ ') + p.price.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      if (p.change !== undefined) {
        changeHtml = `<span class="${p.change >= 0 ? 'change-up' : 'change-down'}">${p.change >= 0 ? '+' : ''}${p.change.toFixed(2)}%</span>`;
      }
    }
    ...
    html += `<tr class="${isDef ? 'row-target' : quarantined ? 'row-quarantine' : ''}">
      ...
      <td class="td-r"><input class="inline-input inline-input--target" type="text"
        value="${item.target !== undefined ? item.target : ''}" data-class="${key}" data-idx="${idx}"
        data-field="target" placeholder="auto" inputmode="decimal" title="Meta do ativo na classe"></td>
      <td class="td-action"><button class="remove-btn" data-class="${key}" data-idx="${idx}" title="Remover ${item.id}">
        <i data-lucide="x" style="width:12px;height:12px"></i></button></td></tr>`;
  });

  html += `<tr class="add-row" data-add-class="${key}"><td colspan="7">+ Adicionar ativo</td></tr></tbody></table></div>`;
  return html;
}
